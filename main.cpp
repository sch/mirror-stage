...
//inverse of above
GLfloat
invgetscalevisit(int s)
{
    switch (s)
    {
    case 0: return 0.02f; break;
    case 1: return 0.1f; break;
    case 2: return 0.2f; break;
    case 3: return 0.3f; break;
    case 4: return 0.4f; break;
    case 5: return 0.6f; break;
    case 6: return 0.7f; break;
    case 7: return 0.15f; break;
    case 8: return 0.25f; break;
    case 9: return 0.03f; break;
    case 10: return 0.06f; break;
    case 11: return 0.0025f; break;
    case 12: return 0.005f; break;
    case 13: return 0.01f; break;
    }
    ...
    case e_Sentry:
        {
            ...
            {
                if (lineSegmentIntersection(x1, y1, x2, y2,
 ...
 * **************/
void
drawEntity(Entity E, GLfloat a, bool par)
{
    switch (E.type)
    {
    case e_EmptyRoom:
...
void
RenderRoom(int room_t, int room_f, int edge_t, int edge_f, bool parity,
           bool globparity, GLfloat depth, GLfloat realdepth, GLfloat outers)
{
    glPushMatrix();
    ...
    */
    ...
    for (int i = shapesize[toshape] - 1; i >= 0; i--)
    {
        if (i != edge_t)
        {
            if (level.rooms[room_t].portal[i]
                &&
                (depth >= 1)
                &&
                realdepth < (MAXREALDEPTH - 1)
                &&
                !level.rooms[room_t].localportal[i])
            {
...
/*
 * This calculates the scale-changes between each portal. Originally
 * I calculated this dynamically, but given that there's no portal
 * gun, it is neater to calculate it statically.
 */
...
/* Counts the number of portals in each room; used in rendering
 * calculations.
 */
void
countPortals()
...
void
draw_editmode(bool resetright, bool resetleftbounds)
{
...

    {
        ...
        if (curtitleselection2 >= 0) {
            ...
            if (currange3_end > customEpisodeList[curtitleselection2].num_episodes
                ||
                ((currange3_end - currange3_begin) < 9
                 &&
                 currange3_end < customEpisodeList[curtitleselection2].num_episodes))
            {
                curtitleselection3 = 0;
                currange3_begin = 0;
    ...
    case ts_editchapterselect:
        ...
        if (curtitleselection2 >= 0) {
            ...
            if (afterselection3
                &&
                ((curtitleselection3 == customEpisodeList[curtitleselection2].num_episodes - 1)
                 ||
    ...
    case ts_editgameselect:
        ...
        print_straight_text(curtitleselection2 >= 0 ? "edit chapter" : "new chapter");
        ...
        for (int i = currange2_begin; i < currange2_end; i++)
        {
            ...
            string chapter_name = customEpisodeList[i].name;

            chapter_name.resize(13);

            //13 CHARACTER LIMIT ON CHAPTER NAMES
            print_straight_text(chapter_name);
        }
        glPopMatrix();
        glPopMatrix();
...
void draw_title()
{
    ...
    if (resetlistbounds) {
        ...
        currange2_begin    = 0;
        currange3_begin    = 0;
        currange2_end      = std::min(curtitleselection2 + 10, (int)episode_list.size());
        currange3_end      = std::min(10, episode_list.num_episodes);
    }
