...
void
closest_point(GLfloat x1 ,GLfloat y1,
              GLfloat x2, GLfloat y2,
              GLfloat x3, GLfloat y3,
              GLfloat* x4, GLfloat* y4)
{
    GLfloat u = ((x3-x1) * (x2-x1) + (y3-y1) * (y2-y1)) / lengthsq(x2-x1, y2-y1);
    // GLfloat u = dot_product(x3 - x1, y3 - y1, x2 - x1, y2 - y1) / square_distance(x2 - x1, y2 - y1);
    *x4 = x1 + u * (x2 - x1);
    *y4 = y1 + u * (y2 - y1);
}
                  ...
                  GLfloat px, GLfloat py, GLfloat qx, GLfloat qy)
{
    ...
    // delete above when `point_intersects_triangle` is finished
